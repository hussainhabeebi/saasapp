...
out of it. The Review Request module and Referral tracking below are the one exception: both
invented event-log-shaped data (who was asked for a review and when, who referred whom) that no
other part of the app reads — so instead of adding more manually-created NocoDB columns for data
nothing else needs, that data lives in **Cloudflare D1** (`env.DB`), a real SQL database bound
directly to the Worker, schema-versioned via `cloudflare-worker/migrations/*.sql` rather than
NocoDB's "add this field by hand in the UI" process. Both modules still read Stage/DealValue/
ClosedAt/Name/Phone/etc. straight out of NocoDB where they need it — D1 only holds what's genuinely
new.
...
3. `wrangler d1 migrations apply leadvyne-d1 --remote`
